...
import { fetchSocieties, type Society } from '@/lib/supabase'
...
export default function SocietiesPage() {
  ...
  const hoveredSociety = hovered ? societies.find((s) => s.name === hovered) : null
  const others = hoveredSociety ? societies.filter((s) => s.name !== hovered) : societies
    ...
    <div className="flex" style={{ height: 'calc(100vh - 57px)' }}>
      ...
      <div className="w-80 xl:w-96 border-r border-gray-200 flex flex-col bg-white shrink-0">
        <div className="px-4 py-3 border-b border-gray-100 text-sm text-gray-500">
          {loading ? 'Loading…' : `${societies.length} societies · sorted by distance to Hexaware`}
        </div>
        {error && (
          ...
          {societies.map((s) => (
            <div
              ...
              className={cn(
                ...
                hovered === s.name ? 'bg-blue-50' : 'hover:bg-gray-50',
              )}
            >
          ...
          {others
            ...
            .map((s) => (
              <CircleMarker
                key={s.name}
                center={[s.lat, s.lng]}
                radius={s.active_count > 0 ? 7 : 5}
                pathOptions={{
                  color:       s.active_count > 0 ? '#2563eb' : '#9ca3af',
                  fillColor:   s.active_count > 0 ? '#2563eb' : '#9ca3af',
                  fillOpacity: s.active_count > 0 ? 0.75 : 0.35,
                  weight: 1,
                }}
              >
